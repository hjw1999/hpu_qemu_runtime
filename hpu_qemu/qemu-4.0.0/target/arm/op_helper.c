...
void HELPER(trace_insn_1)(uint32_t x)
{
    printf("TRACE INSN %x\n", x);
}

void HELPER(trace_peep)(CPUARMState *env)
{
    int x = env->regs[15];
    printf("PC = %x\n", x);
    printf("TRACE INSN\n");
...
void HELPER(trace_dead)(CPUARMState *env)
{
    int x = env->xxregs[0];
    printf("x0 = %x\n", x);
    env->xxregs[0] = 0xdeadbeef;
}

void HELPER(trace_ldst)(CPUARMState *env)
{
    uint32_t x;
    uintptr_t ra = GETPC();
    uint32_t addr = 0x00010000;
    x = cpu_ldq_data_ra(env, addr, ra);
    printf("addr=%x ra=%lx x=%x\n",addr, ra, x); 
}